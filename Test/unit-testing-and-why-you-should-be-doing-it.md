...
Trong procedural programming, một unit có thể là một individual program, function,
...
Stub giả vờ là logging service bằng cách thực hiện interface, nhưng việc thực
hiện đó thực sự không làm gì cả.

   Hơn nữa, trong khi một stub có thể trả về data để giữ cho Class Under Test(CUT)
chạy, nó không bao giờ có thể thực hiện bất kỳ hành động nào sẽ thất trong một
test case.Nếu có, thì nó không còn là stub, và nó trở thành một mock.

   Định nghĩa: Một Stub là một fake không có ảnh hưởng đến việc vượt qua hoặc
thất bại của test case và nó tồn tại hoàn toàn để cho phép test case chạy.
...
đó chúng cung cấp một triển khai fake của một dependency cần thiết của Class Under Test(CUT).
   ...
   Tuy nhiên, chúng vượt xa hơn là chỉ là **stub** bằng cách ghi lại sự tương tác
giữa chính nó và Class Under Test(CUT). Một mock giữ một bản ghi tất cả các tương
tác với Class Under Test(CUT) và báo cáo lại, vượt qua test nếu Class Under
Test(CUT) xử lý đúng và test faild nếu thực hiện không đúng.
...
test có vượt qua hay không.
  ...
  Giả sử bạn có một class được gọi là `WidgetProcessor`. Nó có hai dependency,
`ILogger` và `IVerifier`. Để test `WidgetProcessor`, bạn cần fake cả hai
dependency đó.

  Tuy nhiên, để test `WidgetProcessor`, bạn sẽ muốn thực hiện hai test - một trong
đó bạn stub `ILogger` và test sự tương tác với `IVerifier`, và một thử nghiệm
khác khi bạn stub `IVerifier` và test sự tương tác với `ILogger`.

  Cả hai đều yêu cầu fake, nhưng trong mỗi trường hợp, bạn sẽ cung cấp một stub
class cho class này và mock calss cho class kia.
...
luận; Bạn có thể viết một triển khai `ILogger` trống hoặc bạn sử dụng một
...
test sẽ vượt qua. Nếu không, test faild.
...
mà không cần dependency bên ngoài.

  Định nghĩa: Unit testing là test một simple code entity khi tách biệt hoàn toàn
với các dependency của nó.
...
### Unit testing sẽ tránh bug.
  ...
  Một suite unit testing đầy đủ và kỹ lưỡng sẽ giúp đảm bảo rằng bất kỳ bug nào
...
cho thấy nó vào lần tiếp theo bạn chạy test. Nếu bạn tìm thấy một bug nằm ngoài
các test case, bạn có thể viết một test case cho nó để đảm bảo rằng bug không
bao giờ quay trở lại.

### Unit testing tiết kiệm thời gian.

  Việc viết unit test có tiết kiệm thời gian hay không gây tranh cãi nhất về
unit testing. Hầu hết các developer tin rằng viết unit testing mất nhiều thời
gian hơn là nó tiết kiệm. Tôi không nghĩ rằng là như vậy - thực tế, tôi lập luận ngược lại.

  Viết unit test giúp đảm bảo rằng code của bạn hoạt động đúng như thiết kế,
ngay từ đầu. Các unit testing xác định những gì code của bạn nên làm, và do đó
...
rằng những thay đổi tiếp theo không phá vỡ mọi thứ.
...
### Unit testing mang lại sự yên tâm

  Có một suite test đầy đủ và kỹ lưỡng bao gồm các chức năng hoàn chỉnh của code
của bạn có thể khó đạt được, nhưng có nó sẽ giúp bạn yên tâm.

  Bạn có thể chạy tất cả các suite test đó và biết rằng code của bạn hoạt động
như mong muốn. Bạn có thể cấu trúc lại và thay đổi code, biết rằng nếu bạn phá
vỡ bất cứ điều gì, bạn sẽ biết ngay lập tức.
...
cải thiện nó mà không sợ một điều gì đó sảy ra.
  ...
  Nhiều người trong chúng ta có đoạn code mà chúng ta sợ chạm vào, nhưng với
unit test, bạn sẽ không có loại code đó. Unit test loại bỏ nỗi sợ chạm vào code.

### Unit testing là tài liệu sử dụng đúng cách của một class.

  Một trong những lợi ích của unit test là các test case của bạn có thể xác định
cho các developer tiếp theo cách sử dụng class.
...
## Làm thế nào để cải thiện unit testing
  ...
  Có một số điểm cần lưu ý trong khi thực hiện unit test. Sử dụng các quy ước
đặt tên nhất quán và test một code tại một thời điểm. Đảm bảo rằng có một trường
hợp unit test tương ứng cho một module nếu có bất kỳ thay đổi nào trong code.
